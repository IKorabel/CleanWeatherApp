...
class WeatherApiManager {
    ...
    func getCityName(lat: String, long: String) {
        let cityNameGeocodingURL = "http://api.openweathermap.org/geo/1.0/reverse?lat=\(lat)&lon=\(long)&limit={limit}&appid=\(openWeatherApiKey)"
        
        guard let url = URL(string: cityNameGeocodingURL) else { return }
        var request = URLRequest(url: url, timeoutInterval: Double.infinity)
        request.httpMethod = "GET"
